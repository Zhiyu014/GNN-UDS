...
class Emulator:
    ...
    def build_network(self,conv=None,resnet=False,recurrent=None):
        ...
        inp = [X_in,B_in]
        if conv:
            Adj_in = Input(shape=(self.n_node+self.n_edge if self.graph_base else self.n_node,))
            inp += [Adj_in]
            net = self.get_conv(conv)
            if self.act and self.use_adj and not self.graph_base:
    ...
    def post_proc(self,preds,a):
        ...
        if self.act:
            ...
            fl = self.pump*tf.matmul(tf.cast(preds[...,0]>0.01,tf.float32) * tf.cast(self.area>0,tf.float32),tf.clip_by_value(self.node_edge,0,1))/self.norm_e[0,:,2]
    ...
    def simulate(self,states,runoff,a=None,edge_states=None):
        ...
        for idx,bi in enumerate(runoff):
            ...
            y,ey = y
            ey = self.normalize(squeeze(ey,0).numpy(),'e',True)
    ...
    def predict_tf(self,states,b,a=None,edge_state=None):
        ...
        if self.act:
            # regulate pumping flow (rated value if there is volume in inlet tank)
            fl = self.pump*tf.matmul(tf.cast(y[...,0]>0.01,tf.float32)*tf.cast(self.area>0,tf.float32),tf.clip_by_value(self.node_edge,0,1))
            flow = tf.expand_dims(ey[...,-1] * tf.cast(fl==0,tf.float32) + fl,axis=-1)
            # regulate flow with setting
            ey = concat([ey[...,:-1],tf.multiply(flow,ae)],axis=-1)
            if not self.edge_fusion:
                # regulate pumping flow (rated value if there is volume in inlet tank)
                fli = self.pump_in * tf.cast(y[...,0]>0.01,tf.float32) * tf.cast(self.area>0,tf.float32)/self.norm_y[0,:,1]
                flo = self.pump_out * tf.cast(y[...,0]>0.01,tf.float32) * tf.cast(self.area>0,tf.float32)/self.norm_y[0,:,2]
